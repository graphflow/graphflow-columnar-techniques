...
//            joiner.add(String.format("cypher query   : %s", query.query));
//            joiner.add(String.format("plan index     : %s", query.planIdx));
...
//        logger.info(String.format("Plan: %s", plan));
    ...
    protected static void executeAllPlans(List<QueryExecutionStat> stats,
        List<RegularQueryPlan> plans, int warmUpRuns, int runs) throws InterruptedException {
        System.out.println("total_plans = " + plans.size());
        var planIdx = 0;
        for (var plan: plans) {
            System.out.println("planIdx = " + planIdx);
            var queryExecutionStat = new QueryExecutionStat();
            queryExecutionStat.i = planIdx;
